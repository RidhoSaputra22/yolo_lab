    ...
    const pageMeta = useMemo(() => route === "footage"
        ...
        : route === "tester"
            ? {
                ...
                title: "YOLO Tester",
                description: "Jalankan inferensi video offline, simpan output, dan cek artifact hasil tracking dari satu workspace.",
            }
            : route === "training"
                ? {
                    eyebrow: "Dataset Training Pipeline",
                    title: "YOLO Training",
                    description: "Siapkan split dataset dari label aktif, preview command, lalu jalankan training YOLO dengan konfigurasi yang tersimpan.",
                }
                : {
                    eyebrow: "Frame Annotation Workspace",
                    title: "Manual Labeler",
                    description: "Label bounding box frame CCTV secara manual dengan navigasi cepat, checkpoint, dan auto-label pendukung.",
                }, [route]);
    return (React.createElement(ToastProvider, null,
        React.createElement("div", { className: joinClasses(isLabelerRoute ? "flex pb-32 flex-col overflow-hidden" : "min-h-screen", "yolo-app-shell text-base-content") },
            React.createElement("header", { className: "yolo-app-header sticky top-0 z-30 border-b border-base-300/70 bg-base-100/90 backdrop-blur-xl" },
                React.createElement("div", { className: "mx-auto flex w-full max-w-[1800px] flex-col gap-4 px-4 py-4 lg:px-6" },
                    React.createElement("div", { className: "flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between" },
                        React.createElement("div", { className: "min-w-0 space-y-2 xl:max-w-[560px] 2xl:max-w-4xl" },
                            React.createElement("p", { className: "yolo-eyebrow text-[11px] font-semibold uppercase tracking-[0.28em] text-primary" }, pageMeta.eyebrow),
                            React.createElement("div", { className: "max-w-4xl" },
                                React.createElement("h1", { className: "text-3xl font-bold tracking-tight text-base-content" }, pageMeta.title),
                                React.createElement("p", { className: "mt-2 max-w-3xl text-sm leading-6 text-base-content/70" }, pageMeta.description))),
                        React.createElement("nav", { className: "flex flex-wrap gap-2 lg:justify-end" }, NAV_ITEMS.map((item) => (React.createElement(Button, { key: item.route, variant: route === item.route ? item.variant : "ghost", isSubmit: false, className: joinClasses("yolo-nav-button rounded-md px-4", route !== item.route && "border border-base-300 bg-base-100/80"), onClick: () => navigate(item.route) }, item.label))))))),
