...
export function LogDock({ eyebrow, title, emptyMessage, logs = [], state = "idle", running = false, accentClass = "text-amber-700", }) {
    ...
    return (React.createElement("div", { ref: dockRef, className: "fixed inset-x-0 bottom-0 z-30 border-t border-base-300 bg-base-100/90 shadow-2xl backdrop-blur-xl" },
        React.createElement("div", { className: "w-full" },
            React.createElement("div", { className: "overflow-hidden border border-base-300 bg-base-100/90 shadow-lg md:rounded-t-md" },
                React.createElement("div", { className: "flex flex-wrap items-center justify-between gap-3 px-4 py-3" },
                    ...
                    React.createElement("div", { className: "flex flex-wrap items-center gap-2" },
                        ...
                        React.createElement(Button, { variant: "ghost", isSubmit: false, size: "sm", className: "rounded-md border border-base-300 bg-base-100/80 px-4", onClick: () => setCollapsed((current) => !current) }, collapsed ? "▲" : "▼"))),
                !collapsed && (React.createElement("div", { className: "border-t border-base-300 px-4 pb-4" },
                    React.createElement("pre", { ref: preRef, className: "yolo-code-panel max-h-[30vh] min-h-[160px] overflow-auto rounded-md p-4 text-xs leading-6 text-slate-100" }, logs.length ? logs.join("\n") : emptyMessage)))))));
}
