            ...
            React.createElement("div", { className: "overflow-hidden rounded-sm border border-base-300 bg-base-100/92 shadow-lg" },
                React.createElement("div", { className: "flex flex-wrap items-center justify-between gap-3 px-4 py-3" },
                    ...
                    React.createElement("div", { className: "flex flex-wrap items-center gap-2" },
                        ...
                        React.createElement(Button, { variant: "ghost", isSubmit: false, size: "sm", className: "rounded-sm border border-base-300 bg-base-100 px-4", onClick: () => setCollapsed((current) => !current) }, collapsed ? "▲" : "▼"))),
                !collapsed && (React.createElement("div", { className: "border-t border-base-300 px-4 pb-4" },
                    React.createElement("pre", { ref: preRef, className: "max-h-[30vh] min-h-[160px] overflow-auto rounded-sm bg-slate-950 p-4 text-xs leading-6 text-slate-100" }, logs.length ? logs.join("\n") : emptyMessage)))))));
}
