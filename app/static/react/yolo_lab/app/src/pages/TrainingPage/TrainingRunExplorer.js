...
export function TrainingRunExplorer({ runs, selectedRun, selectedRunKey, onSelectRun, }) {
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement("div", { className: "grid min-w-0 gap-4 xl:grid-cols-[380px_minmax(0,1fr)]" },
            React.createElement(Card, { className: "h-fit min-w-0 rounded-md border border-base-300 bg-base-100/90 shadow-xl xl:sticky xl:top-28" },
                React.createElement("div", { className: "space-y-4" },
                    ...
                    React.createElement("div", { className: "grid gap-2 overflow-auto" }, runs.length ? (runs.map((run) => (React.createElement("button", { key: run.key, type: "button", onClick: () => onSelectRun(run.key), className: joinClasses("w-full overflow-hidden rounded-md border px-3 py-3 text-left transition duration-150", run.key === selectedRunKey
                        ...
                        React.createElement("p", { className: "text-wrap-anywhere text-sm font-semibold leading-5 text-slate-900" }, run.key))))) : (React.createElement(Alert, { type: "info", className: "rounded-md text-sm" }, "Belum ada run training di folder output."))))),
            React.createElement(Card, { className: "min-w-0 rounded-md border border-base-300 bg-base-100/90 shadow-xl" },
                React.createElement("div", { className: "space-y-4" },
                    React.createElement("div", { className: "flex flex-wrap items-start justify-between gap-3" },
                        React.createElement("div", null,
                        ...
                        selectedRun ? (React.createElement("div", { className: "flex flex-wrap items-center justify-end gap-2" },
                            React.createElement(Button, { variant: "ghost", isSubmit: false, size: "sm", className: "rounded-md border border-base-300 bg-base-100 px-4", disabled: !imageArtifacts.length, onClick: () => setIsAssetModalOpen(true) },
                                "Asset (",
                    ...
                    selectedRun ? (React.createElement(React.Fragment, null,
                        React.createElement("div", { className: "grid gap-3 md:grid-cols-4" },
                            React.createElement("div", { className: "rounded-md border border-base-300 bg-base-200/40 p-3 md:col-span-2" },
                            ...
                            React.createElement("div", { className: "rounded-md border border-base-300 bg-base-200/40 p-3" },
                        ...
                        ].map(({ label, value, tone }) => (React.createElement("div", { key: label, className: `rounded-md border border-base-300 bg-base-200/40 p-3 ${tone || ""}` },
                        ...
                        ].map(({ label, value, detail }) => (React.createElement("div", { key: label, className: "rounded-md border border-base-300 bg-base-200/40 p-3" },
                            React.createElement("p", { className: "text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-500" }, label),
                            React.createElement("p", { className: "mt-2 text-base font-semibold text-slate-900" }, value),
                        ...
                        React.createElement("div", { className: "flex flex-wrap gap-2" },
                            selectedRun.artifacts?.bestWeights?.downloadUrl ? (React.createElement("a", { href: selectedRun.artifacts.bestWeights.downloadUrl, target: "_blank", rel: "noreferrer", className: "rounded-md bg-warning px-4 py-2 text-sm font-semibold text-white hover:bg-warning/90" }, "Best Weights")) : null,
                            selectedRun.artifacts?.lastWeights?.downloadUrl ? (React.createElement("a", { href: selectedRun.artifacts.lastWeights.downloadUrl, target: "_blank", rel: "noreferrer", className: "rounded-md border border-base-300 bg-base-100 px-4 py-2 text-sm font-semibold hover:bg-base-200" }, "Last Weights")) : null,
                            selectedRun.artifacts?.resultsCsv?.downloadUrl ? (React.createElement("a", { href: selectedRun.artifacts.resultsCsv.downloadUrl, target: "_blank", rel: "noreferrer", className: "rounded-md bg-info px-4 py-2 text-sm font-semibold text-white hover:bg-info/90" }, "results.csv")) : null,
                            selectedRun.artifacts?.argsYaml?.downloadUrl ? (React.createElement("a", { href: selectedRun.artifacts.argsYaml.downloadUrl, target: "_blank", rel: "noreferrer", className: "rounded-md border border-base-300 bg-base-100 px-4 py-2 text-sm font-semibold hover:bg-base-200" }, "args.yaml")) : null),
                        (selectedRun.previewArtifacts || []).length ? (React.createElement("div", { className: "grid gap-4 lg:grid-cols-3" }, selectedRun.previewArtifacts.map((artifact) => {
                            ...
                            return (React.createElement("button", { key: artifact.path, type: "button", disabled: previewIndex < 0, onClick: () => setActiveImageIndex(previewIndex), className: "overflow-hidden rounded-md border border-base-300 bg-base-100 text-left shadow-md transition hover:-translate-y-0.5 hover:border-base-content/20 disabled:cursor-default disabled:hover:translate-y-0" },
                        ...
                        }))) : (React.createElement(Alert, { type: "info", className: "rounded-md text-sm" }, "Run ini belum memiliki artifact preview gambar.")))) : (React.createElement(Alert, { type: "info", className: "rounded-md text-sm" }, "Belum ada detail run yang bisa ditampilkan."))))),
        React.createElement(Modal, { open: isAssetModalOpen, onClose: closeAssetGallery, title: selectedRun ? `Asset Run - ${selectedRun.key}` : "Asset Run", size: "xl" },
            React.createElement("div", { className: "space-y-4" },
                React.createElement("div", { className: "flex flex-wrap items-start justify-between gap-3 rounded-md border border-base-300 bg-base-200/30 p-4" },
                ...
                imageArtifacts.length ? (React.createElement("div", { className: "grid gap-4 sm:grid-cols-2 xl:grid-cols-3" }, imageArtifacts.map((artifact, index) => (React.createElement("button", { key: artifact.path, type: "button", onClick: () => setActiveImageIndex(index), className: "group overflow-hidden rounded-md border border-base-300 bg-base-100 text-left shadow-md transition duration-150 hover:-translate-y-0.5 hover:border-base-content/20" },
                    ...
                    React.createElement("div", { className: "space-y-1 p-3" },
                        ...
                        React.createElement("p", { className: "text-[11px] text-slate-500" }, formatTimestamp(artifact.modifiedAt)))))))) : (React.createElement(Alert, { type: "info", className: "rounded-md text-sm" }, "Run ini belum memiliki asset gambar yang bisa ditampilkan.")))),
                ...
                React.createElement("div", { className: "flex flex-wrap items-start justify-between gap-3" },
                    ...
                    React.createElement("div", { className: "flex flex-wrap items-center gap-2" },
                        imageArtifacts.length > 1 ? (React.createElement(React.Fragment, null,
                            React.createElement("button", { type: "button", className: "rounded-md border border-white/15 bg-white/10 px-3 py-2 text-sm font-semibold text-white hover:bg-white/15", onClick: showPreviousImage }, "Prev"),
                            React.createElement("button", { type: "button", className: "rounded-md border border-white/15 bg-white/10 px-3 py-2 text-sm font-semibold text-white hover:bg-white/15", onClick: showNextImage }, "Next"))) : null,
                        React.createElement("a", { href: activeImageArtifact.downloadUrl, target: "_blank", rel: "noreferrer", className: "rounded-md border border-white/15 bg-white/10 px-3 py-2 text-sm font-semibold text-white hover:bg-white/15" }, "Buka Tab Baru"),
                        React.createElement("button", { type: "button", className: "rounded-md bg-white px-3 py-2 text-sm font-semibold text-slate-900 hover:bg-slate-100", onClick: () => setActiveImageIndex(-1) }, "Tutup"))),
                React.createElement("div", { className: "flex min-h-0 flex-1 items-center justify-center overflow-auto rounded-md border border-white/10 bg-black/40 p-3 md:p-6" },
