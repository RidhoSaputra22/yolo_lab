...
export function FootageImportPanel({ fileInputRef, selectedFiles, isImporting, targetDir, onFileChange, onImport, }) {
    return (React.createElement(Card, { className: "rounded-md border border-base-300 bg-base-100/90 shadow-lg" },
        React.createElement("div", { className: "space-y-4" },
            ...
            React.createElement("div", { className: "rounded-md border border-dashed border-base-300 bg-base-200/30 p-4" },
            ...
            selectedFiles.length > 0 ? (React.createElement("div", { className: "grid gap-2" },
                selectedFiles.slice(0, 5).map((file) => (React.createElement("div", { key: `${file.name}-${file.size}-${file.lastModified}`, className: "flex items-center justify-between gap-3 rounded-md border border-base-300 bg-base-100 px-3 py-2" },
            ...
            React.createElement("div", { className: "flex justify-end" },
                React.createElement(Button, { variant: "info", isSubmit: false, className: "rounded-md px-5", disabled: !selectedFiles.length, loading: isImporting, onClick: onImport }, "Import Footage")))));
}
