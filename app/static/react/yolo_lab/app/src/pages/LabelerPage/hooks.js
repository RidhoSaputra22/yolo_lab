...
import { getDisplayMetricsForZoom, getStageLayoutMetrics, } from "./utils.js";
...
export function useCanvasGeometry() {
     ...
     */
    const toImagePointFromEvent = useCallback((event, displayMetrics, naturalSize) => {
        const canvas = overlayRef.current;
        if (!canvas) {
            return { x: 0, y: 0 };
        }
    ...
    const getHandleHit = useCallback((point, box, scale) => {
        ...
        for (const [name, handle] of Object.entries(handles)) {
            const hit = Math.abs(point.x - handle.x) <= threshold && Math.abs(point.y - handle.y) <= threshold;
            if (hit) {
                return name;
            }
        }
     ...
     */
    const getCursorForPoint = useCallback((point, boxes, scale) => {
...
export function useZoomInteraction() {
    const zoomAtViewportPoint = useCallback((stageViewportRef, frameShellRef, naturalSize, stageSize, zoomLevel, getNextZoom) => {
        ...
        requestAnimationFrame(() => {
            ...
            nextViewport.scrollLeft = clamp(nextStageLayout.frameOffsetX + imagePointX * nextScaleX - pointerX, 0, maxScrollLeft);
            nextViewport.scrollTop = clamp(nextStageLayout.frameOffsetY + imagePointY * nextScaleY - pointerY, 0, maxScrollTop);
        });
        return nextZoom;
    }, []);
    return {
