...
export default function FootagePage({ onNavigate }) {
    ...
    useEffect(() => {
        ...
        setFormValues((current) => (current.framesDir === nextFramesDir
            ? current
            : { ...current, framesDir: nextFramesDir }));
        setDefaults((current) => (current.framesDir === nextFramesDir
            ? current
            : { ...current, framesDir: nextFramesDir }));
    ...
    const stateBadgeType = job?.state === "failed"
        ? "error"
        : job?.state === "finished"
            ? "success"
            : job?.running
        ...
        React.createElement("section", { className: "yolo-toolbar flex flex-col gap-3 rounded-md border border-base-300 bg-base-100/90 p-4 shadow-lg xl:flex-row xl:items-center xl:justify-between" },
            React.createElement("div", { className: "flex flex-wrap items-center gap-3" },
                ...
                React.createElement("div", { className: "text-sm" },
                    React.createElement("span", { className: "font-semibold text-base-content" }, library?.footageDir || defaults.footageDir || "-"),
            ...
            React.createElement("div", { className: "flex flex-wrap items-center gap-2" },
                React.createElement(Button, { variant: "info", isSubmit: false, size: "sm", className: "rounded-md px-5", disabled: Boolean(job?.running) || isConfigLoading, onClick: handleRun }, "\u25B6 Ekstrak Frame"),
