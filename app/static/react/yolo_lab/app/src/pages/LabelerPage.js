...
export default function LabelerPage() {
    ...
    const handleImportFrameArchive = async ({ file, targetSubdir }) => {
        ...
        if (dirtyRef.current) {
            setNotice(createNotice("warning", "Frame aktif punya perubahan yang belum disimpan. Simpan dulu sebelum import bundle baru."));
            return false;
        }
        if (!file) {
        ...
        try {
            ...
            setNotice(createNotice("success", `${result.imageCount || 0} frame dan ${result.labelCount || 0} label berhasil diimport ke ${result.framesDir}.`));
            return true;
        }
        catch (error) {
            setNotice(createNotice("error", error.message));
    ...
    const handleExportFrameArchive = async () => {
        ...
        if (dirtyRef.current) {
            setNotice(createNotice("warning", "Frame aktif punya perubahan yang belum disimpan. Simpan dulu supaya isi zip sinkron dengan label di disk."));
            return false;
        }
        ...
        try {
            ...
            const archiveName = getDownloadFilename(response.headers.get("content-disposition"), `yolo-lab-frames-${Date.now()}.zip`);
    ...
    const handleDeleteCurrentFrame = async () => {
        ...
        const labelWarning = currentItem?.hasLabelFile
            ...
            : "";
        const confirmed = window.confirm(`Hapus frame aktif "${imageName}"${labelWarning}?${checkpointWarning}${dirtyWarning}\n\nAksi ini tidak bisa dibatalkan.`);
        if (!confirmed) {
            return false;
        }
        ...
        try {
            ...
            if (!refreshedConfig) {
                const nextCheckpointImage = typeof result.checkpointImage === "string" && result.checkpointImage.trim()
                ...
                clearActiveFrameState();
                setNotice(createNotice("warning", `Frame ${imageName} sudah dihapus, tetapi workspace gagal dimuat ulang. Klik Refresh untuk sinkron ulang.`));
                return false;
            }
            ...
            setNotice(createNotice("success", result.labelDeleted
    ...
    const replaceAutolabelSelection = (imageNames) => {
        ...
        const nextSelection = Array.from(new Set((Array.isArray(imageNames) ? imageNames : [])
    ...
    const toggleAutolabelSelection = (imageName) => {
        ...
        setAutolabelSelectedImages((current) => current.includes(normalizedImageName)
    ...
    const handleAutolabelSelection = async () => {
        ...
        try {
            ...
            setAutolabelJob(nextJob);
            setIsAutolabelModalOpen(false);
            setNotice(createNotice("info", `Auto-label ${targetImages.length} frame terpilih dimulai. Panel labeling dikunci sampai selesai.`));
        }
        catch (error) {
            setNotice(createNotice("error", error.message));
        }
    ...
    return (React.createElement(React.Fragment, null,
        React.createElement("div", { className: "grid h-[75vh] min-h-0 gap-4 grid-rows-[minmax(280px,42vh)_minmax(0,1fr)] lg:grid-cols-[360px_minmax(0,1fr)] lg:grid-rows-1 " },
            ...
            React.createElement("section", { className: "min-h-0" },
                React.createElement(LabelerCanvas, { currentImageName: currentImageName, imageSrc: imageSrc, stageViewportRef: stageViewportRef, frameShellRef: frameShellRef, overlayRef: overlayRef, displayMetrics: displayMetrics, stageLayout: stageLayout, zoomLabel: zoomLabel, zoomLevel: zoomLevel, minZoomLevel: MIN_ZOOM_LEVEL, maxZoomLevel: MAX_ZOOM_LEVEL, interactionDisabled: isLabelingLocked, onZoomIn: zoomIn, onZoomOut: zoomOut, onResetZoom: resetZoom, onDeleteCurrentFrame: handleDeleteCurrentFrame, deleteDisabled: Boolean(isLoading || isLabelingLocked || isDeletingCurrentFrame || !currentImageName), deleteBusy: isDeletingCurrentFrame, onCanvasMouseDown: handleCanvasMouseDown }))),
        React.createElement(LabelerLogs, { job: autolabelJob }),
        React.createElement(LabelerAutolabelModal, { open: isAutolabelModalOpen, onClose: () => setIsAutolabelModalOpen(false), activeFramesDir: activeFramesDir, images: images, visibleImages: visibleImages, totalImages: images.length, currentImageName: currentImageName, selectedImageNames: autolabelSelectedImages, autolabelConfig: autolabelConfig, autolabelSuggestions: autolabelSuggestions, autolabelWarnings: autolabelWarnings, job: autolabelJob, onAutolabelConfigChange: setAutolabelConfig, onReplaceSelection: replaceAutolabelSelection, onSelectCurrentImage: selectCurrentAutolabelImage, onSelectVisibleImages: selectVisibleAutolabelImages, onSelectPendingImages: selectPendingAutolabelImages, onClearSelection: () => setAutolabelSelectedImages([]), onToggleImageSelection: toggleAutolabelSelection, onAutolabelCurrent: handleAutolabelCurrent, onAutolabelSelection: handleAutolabelSelection, onAutolabelAll: handleAutolabelAll }),
        React.createElement(LabelerArchiveModal, { open: isArchiveModalOpen, initialMode: archiveModalMode, onClose: () => setIsArchiveModalOpen(false), activeFramesDir: activeFramesDir, activeLabelsDir: activeLabelsDir, images: images, classNames: classNames, currentImageName: currentImageName, disabled: Boolean(isLoading || isLabelingLocked || dirty), warningMessage: archiveWarning, onImport: handleImportFrameArchive, onExport: handleExportFrameArchive, N: true })));
}
