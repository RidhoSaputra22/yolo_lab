...
export default function TrainingPage() {
  ...
  useEffect(() => {
    if (!runs.length) { setSelectedRunKey(""); return; }
  ...
  const selectedRun = useMemo(
    () => runs.find((run) => run.key === selectedRunKey) || runs[0] || null,
    [runs, selectedRunKey],
  );
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(async () => {
      ...
      try {
        ...
        if (!cancelled) { setPreview(nextPreview); setPreviewError(""); setPreviewState("ready"); }
      } catch (error) {
        if (!cancelled) { setPreviewError(error.message); setPreviewState("error"); }
      }
    }, PREVIEW_DEBOUNCE_MS);
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="text-sm">
            <span className="font-semibold text-slate-900">
              {job?.activeRunName || preview?.config?.runName || defaults.runName || "-"}
            </span>
            <span className="ml-2 text-slate-500">
              {job?.durationSeconds != null ? `${job.durationSeconds}s` : ""} • {formatCount(runs.length, "run")}
            </span>
          </div>
        </div>
